...
try:
    from autogen.oai.bedrock import BedrockClient, calculate_cost
...
@pytest.fixture
def client():
    return BedrockClient(profile_name="sbx-eng", region_name="us-east-1")


skip_reason = "Bedrock dependency is not installed"
...
#     # Missing any api_key
#     with pytest.raises(RuntimeError) as assertinfo:
#         BedrockClient()  # Should raise an AssertionError due to missing api_key
...
#     # Creation works
#     BedrockClient(profile_name="sbx-eng", region_name="us-east-1")  # Should create okay now.


# Test standard initialization
@pytest.mark.skipif(skip, reason=skip_reason)
def test_valid_initialization(client):
    assert client.bedrock_runtime is not None, "Cannot initialize boto3 session"


# Test parameters
# @pytest.mark.skipif(skip, reason=skip_reason)
# def test_parsing_params(client):
#     # All parameters
#     params = {
...
#         # "top_p": 0.8,
#     }
#     result = client.parse_params(params)
...
def test_cost_calculation(mock_response):
    ...
    assert (
        calculate_cost(response.usage["prompt_tokens"], response.usage["completion_tokens"], response.model) == 0.0056
    )
...
def test_create_response(mock_chat, client):
    # Mock BedrockClient response
    mock_response = MagicMock()
    mock_response.choices = [
        MagicMock(finish_reason="stop", message=MagicMock(content="Example response", tool_calls=None))
    ]
    mock_response.id = "mock_response_id"
    mock_response.model = "mistral.mistral-large-2402-v1:0"
    mock_response.usage = MagicMock(prompt_tokens=10, completion_tokens=20)  # Example token usage
    ...
    mock_response = client.create(params)

    # Assertions to check if response is structured as expected
    assert (
        mock_response.choices[0].message.content == "Example response"
    ), "Response content should match expected output"
    assert mock_response.id == "mock_response_id", "Response ID should match the mocked response ID"
    assert (
        mock_response.model == "mistral.mistral-large-2402-v1:0"
    ), "Response model should match the mocked response model"
    assert mock_response.usage.prompt_tokens == 10, "Response prompt tokens should match the mocked response usage"
    assert (
...
@pytest.mark.skipif(skip, reason=skip_reason)
@patch("autogen.oai.bedrock.BedrockClient.create")
def test_create_response_with_tool_call(mock_chat, client):
    ...
    response = client.create(
        {"messages": groq_messages, "tools": converted_functions, "model": "mistral.mistral-large-2402-v1:0"}
    )
